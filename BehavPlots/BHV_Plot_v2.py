...
for m, d in enumerate(imaging_days):
    ...
    # Added by C.Zhang   
    # Below added used for plotting in a signal based avg.        
    for k in range(TrialNumber):        
        if (performance[k] == 1 or performance[k] == 3):
            Aud_updateflag= True
            for n in range(1,k):
                if not Aud_SearchDone:
                    if (performance[k-n]==1 or performance[k-n]==3):
                        Aud_StimCounter.append(1)
                    else:
