...
function BehavFiles_Converter(Mouse, Date, inputmainPath, outputmainPath)
%%% Mouse example 'CZ004'
...
subMouse = Mouse;
subDate = Date;
subPath = [subMouse '\' subDate '\'];
Path = [inputmainPath subPath];

% to edit output path
%outputmainPath = 'K:\';
subPathprefix = [subMouse '_' subDate '_'];
outputpath = [outputmainPath subPath] ;
mkdir(outputpath);
...
for i = 1 : size(BehavFiles,1)
  
  ResultsTemp = importdata([Path BehavFiles(i).name '\Results.txt']);
  EarlyLicksTemp = importdata([Path BehavFiles(i).name '\EarlyLicks.txt']);
...
save([outputpath,'BehavResults.mat'],'BehavResults');
